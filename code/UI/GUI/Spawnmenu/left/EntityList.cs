...
public partial class EntityList : Panel
{
	...
	void update2()
	{
		...
		if (tab == "Ammo")
		{
			...
			var ents = TypeLibrary.GetTypes<BaseAmmo>()
		...
		else if (tab == "Items")
		{
			...
			var ents = TypeLibrary.GetTypes<Suit>().ToArray();
			foreach (var entry in ents)
			{
				Canvas.AddItem(entry);
			}

			ents = TypeLibrary.GetTypes<Battery>().ToArray();
			foreach (var entry in ents)
			{
				Canvas.AddItem(entry);
			}

			ents = TypeLibrary.GetTypes<HealthKit>().ToArray();
			foreach (var entry in ents)
			{
				Canvas.AddItem(entry);
			}

			ents = TypeLibrary.GetTypes<item_sodacan>().ToArray();
			foreach (var entry in ents)
			{
				Canvas.AddItem(entry);
			}
